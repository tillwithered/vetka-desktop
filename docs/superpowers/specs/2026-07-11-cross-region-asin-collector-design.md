...
The collector will use a direct cross-region probe first. It constructs the target region's canonical `/dp/{ASIN}` URL from every confirmed listing for the doll, opens it, and applies the existing SKU/EAN/title fact-triangle before accepting the regional price. Only a verified, New offer creates or refreshes that region's confirmed listing. The current SKU-based search remains a fallback for products whose ASIN differs by marketplace.
...
2. Probe `https://{target-host}/dp/{ASIN}` for each ASIN. Parse the product page and require the existing catalog fact-triangle when catalog rules exist.
...
- The matcher remains strict: an ASIN copied from another region still needs the same SKU/EAN/title facts and New condition.
