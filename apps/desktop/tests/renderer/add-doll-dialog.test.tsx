...
describe('AddDollDialog', () => {
  ...
  it('creates a manual card without forcing an Amazon link', async () => {
    ...
    await user.type(screen.getByLabelText('Название'), 'Manual doll');
    await user.click(screen.getByRole('button', { name: 'Сохранить' }));

    expect(create).toHaveBeenCalledWith(expect.objectContaining({ name: 'Manual doll' }));
    expect(addListing).not.toHaveBeenCalled();
  });
