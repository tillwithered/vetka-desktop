...
describe('OfficialStoreImportService', () => {
  ...
  it('imports an official Store doll and its regional price by SKU', async () => {
    ...
    const service = new OfficialStoreImportService({ catalog, priceService, driver, now: () => new Date('2026-07-11T00:00:00.000Z') });
  ...
  it('reports a blocked Store region without deleting or interrupting other regions', async () => {
    ...
    const prices = new PriceRepository(db);
    const priceService = new PriceService({ db, prices, collector: { refreshDoll: vi.fn() }, dataDir: 'C:/data', getRate: () => 650_000_000 });
    const driver = { openStore: vi.fn(async () => '<html data-vetka-collector-status="blocked"></html>'), openProduct: vi.fn(), close: vi.fn(async () => undefined) };
    const service = new OfficialStoreImportService({ catalog, priceService, driver });
