...
export function HomePage() {
  ...
  return <section className="flex flex-1 flex-col gap-6 p-6"><PageHeader title="Избранное" description="Куклы с ценами и быстрым переходом к заказу" actions={<AddDollDialog />} /><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><StatCard label="Избранных кукол" value={dolls.data?.length ?? 0} icon={HeartIcon} /><StatCard label="Нужна проверка" value="0" detail="Автоматическая проверка появится здесь" /></div>{dolls.isLoading ? <div className="rounded-xl border p-6 text-sm text-muted-foreground">Загружаю рабочий список…</div> : dolls.data?.length ? <TableSurface><FavoritePriceTable dolls={dolls.data} onFavorite={(doll) => favorite.mutate(doll)} /></TableSurface> : <EmptyState icon={HeartIcon} title="Избранное пока пусто" description="Добавьте первую куклу или отметьте существующую сердцем." action={<AddDollDialog />} />}</section>;
}
