...
export function HomePage() {
  ...
  const needsCheck = (dolls.data ?? []).filter((_, index) => {
    ...
    return query?.isError || prices.length === 0 || prices.some((price) => price.latestCheckStatus !== 'verified' || isPriceCheckOverdue(price.checkedAt));
  }).length;
  const checkDetail = priceChecksLoading
    ? 'Сверяю свежесть цен'
    : needsCheck > 0
      ? 'Нет цены или проверка просрочена'
      : 'Все цены актуальны';
