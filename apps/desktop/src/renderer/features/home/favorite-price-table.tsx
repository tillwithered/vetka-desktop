...
export function FavoritePriceTable({ dolls, onFavorite }: { dolls: Doll[]; onFavorite(doll: Doll): void }) {
  ...
  const refresh = useMutation({ mutationFn: async (id: string) => unwrap(await window.vetka.amazon.refreshDoll(id, allRegions)), onSuccess: async (_result, id) => { await client.invalidateQueries({ queryKey: ['prices', id] }); toast.success('Цены обновлены'); }, onError: (error) => toast.error(error.message) });
