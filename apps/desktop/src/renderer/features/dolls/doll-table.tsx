...
export function DollTable({ dolls, pricesByDoll = {}, onFavorite }: { dolls: Doll[]; pricesByDoll?: Record<string, CurrentPrice[]>; onFavorite(doll: Doll): void }) {
  return <Table><TableHeader><TableRow><TableHead>Кукла</TableHead><TableHead>Линейка</TableHead><TableHead>Артикул</TableHead><TableHead>Цены</TableHead><TableHead className="w-24 text-right">Действия</TableHead></TableRow></TableHeader><TableBody>{dolls.map((doll) => <TableRow key={doll.id}><TableCell><div className="flex items-center gap-3">{doll.imagePath ? <img className="size-9 rounded-md object-cover" src={doll.imagePath} alt={`Миниатюра ${doll.name}`} /> : <span className="flex size-9 items-center justify-center rounded-md bg-muted text-muted-foreground" aria-label="Нет изображения"><ImageIcon className="size-4" /></span>}<div><Link className="font-medium hover:underline" to={`/dolls/${doll.id}`}>{doll.name}</Link>{doll.characterName && <div className="text-xs text-muted-foreground">{doll.characterName}</div>}</div></div></TableCell><TableCell>{doll.lineName ? <Badge variant="secondary">{doll.lineName}{doll.generation ? ` · ${doll.generation}` : ''}</Badge> : '—'}</TableCell><TableCell className="font-mono text-xs">{doll.mattelSku ?? doll.upcEan ?? '—'}</TableCell><TableCell><PriceCell prices={pricesByDoll[doll.id] ?? []} /></TableCell><TableCell className="text-right"><Button size="icon-sm" variant={doll.isFavorite ? 'secondary' : 'ghost'} aria-label={doll.isFavorite ? 'Убрать из избранного' : 'Добавить в избранное'} onClick={() => onFavorite(doll)}><HeartIcon className={doll.isFavorite ? 'fill-current' : ''} /></Button><Button asChild size="icon-sm" variant="ghost"><Link to={`/dolls/${doll.id}`} aria-label="Открыть карточку"><MoreHorizontalIcon /></Link></Button></TableCell></TableRow>)}</TableBody></Table>;
}
