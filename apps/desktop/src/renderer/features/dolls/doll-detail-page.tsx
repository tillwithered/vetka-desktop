...
export function DollDetailPage() {
  const { id = '' } = useParams(); const [range, setRange] = useState<'7d' | '30d' | '90d' | 'all'>('30d'); const [challenge, setChallenge] = useState<{ requestId: string; region: AmazonRegion } | null>(null); const client = useQueryClient();
  const doll = useQuery({ queryKey: ['doll', id], queryFn: async () => unwrap(await window.vetka.dolls.get(id)) }); const prices = useQuery({ queryKey: ['prices', id], queryFn: async () => unwrap(await window.vetka.prices.current(id)) }); const history = useQuery({ queryKey: ['history', id, range], queryFn: async () => unwrap(await window.vetka.prices.history(id, range)) });
  ...
  useEffect(() => window.vetka.amazon.onProgress((event) => { if (event.stage === 'captcha_required' && event.region) setChallenge({ requestId: event.requestId, region: event.region }); if (event.stage === 'completed' || event.stage === 'failed') setChallenge(null); }), []);
  ...
  return <section className="flex flex-1 flex-col gap-6 p-6"><Button asChild size="sm" variant="ghost" className="w-fit"><Link to="/dolls"><ArrowLeftIcon />К списку</Link></Button><PageHeader title={doll.data.name} description={metadata} meta={doll.data.generation && <Badge variant="secondary">{doll.data.generation}</Badge>} actions={<><Button size="icon-sm" variant={doll.data.isFavorite ? 'secondary' : 'ghost'} aria-label="Избранное" onClick={() => favorite.mutate()}><HeartIcon className={doll.data.isFavorite ? 'fill-current' : ''} /></Button><Button size="sm" variant="secondary" onClick={() => refresh.mutate()} disabled={refresh.isPending}><RefreshCwIcon className={refresh.isPending ? 'animate-spin' : ''} />{refresh.isPending ? 'Проверяю…' : 'Обновить цены'}</Button><CreateOrderSheet prices={prices.data ?? []} /></>} />{challenge && <Alert><AlertTitle>Amazon просит пройти CAPTCHA</AlertTitle><AlertDescription className="flex items-center justify-between gap-4"><span>Завершите проверку в открытом окне браузера.</span><Button size="sm" onClick={async () => { unwrap(await window.vetka.amazon.resumeRegion(challenge.requestId, challenge.region)); setChallenge(null); }}>Продолжить</Button></AlertDescription></Alert>}<div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,1fr)]"><ChartCard title="История цены" description="Сохранённая стоимость на момент проверки"><PriceHistoryChart points={history.data ?? []} range={range} onRangeChange={setRange} /></ChartCard><Card><CardHeader><CardTitle>Регионы Amazon</CardTitle><CardDescription>Подтверждённые предложения в состоянии New</CardDescription></CardHeader><CardContent><RegionalOfferList prices={prices.data ?? []} /></CardContent></Card></div></section>;
}
